...
/**
 * 设置一个固定群，从群中获取元素，用户身份对应G1元素，密文对应g2元素
 * todo:固定化G1\G2\GT
 */
...
//        Pairing bp = PairingFactory.getPairing("a.properties");
        ...
        // 固定 G1 和 G2 的生成元
        Element fixedG1 = G1.newElement().setToRandom().getImmutable();
        Element fixedG2 = G2.newElement().setToRandom().getImmutable();

        // 将生成元保存到文件
        saveElementToFile(fixedG1, "fixedG1.element");
        saveElementToFile(fixedG2, "fixedG2.element");
    ...
    // 保存群元素到文件
    public static void saveElementToFile(Element element, String filename) throws IOException {
        try (FileOutputStream fos = new FileOutputStream(filename);
             ObjectOutputStream oos = new ObjectOutputStream(fos)) {
            oos.writeObject(element.toBytes());
        }
    }
}
